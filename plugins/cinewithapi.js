...
async function downloadAndSend(conn, mek, from, movie, quality, thumbBuf, reply) {
    ...
    try {
        const scraperUrl = `https://cine-fix.vercel.app/api/scrape/?url=${encodeURIComponent(movie.full_url)}`;
        ...
        console.log('📦 Scraper status    :', scraperData?.status);
        console.log('📦 Scraper keys      :', Object.keys(scraperData).join(', '));

        if (!scraperData?.status || !scraperData?.downloads) {
            console.log('❌ Scraper failed! Response:', JSON.stringify(scraperData));
            await reply('❌ No download link found for: ' + movie.title);
            return;
        }

        const qualityKey = quality.toLowerCase(); // '480p', '720p', '1080p'
        const finalUrl = scraperData.downloads[qualityKey];

        if (!finalUrl) {
            console.log(`❌ Quality ${quality} not found. Available:`, Object.keys(scraperData.downloads).join(', '));
            await reply(`❌ *${quality}* not available for this movie.\nAvailable: ${Object.keys(scraperData.downloads).join(', ')}`);
            return;
        }

        console.log(`🔗 Download URL (${quality}):`, finalUrl);
...
cmd({
    ...
    // ─── STAGE: SELECT QUALITY → DOWNLOAD ─────────────────
        ...
        // ✅ Back to select stage — session alive for next download
        sessionMap.set(from, {
            results: session.results,
            stage: 'select'
        });
